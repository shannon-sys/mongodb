...
namespace mongo {
    ...
    RocksEngine::RocksEngine(const std::string& path, bool durable, int formatVersion,
        ...
        {  // create block cache
            ...
            //_block_cache = shannon::NewLRUCache(cacheSizeGB * 1024 * 1024 * 1024LL, 6);
        }
        _maxWriteMBPerSec = rocksGlobalOptions.maxWriteMBPerSec;
        //_rateLimiter.reset(
        //    shannon::NewGenericRateLimiter(static_cast<int64_t>(_maxWriteMBPerSec) * 1024 * 1024));
        //if (rocksGlobalOptions.counters) {
        //    _statistics = shannon::CreateDBStatistics();
        //}
        ...
        shannon::Status s;
        //if (readOnly) {
        //    s = shannon::DB::OpenForReadOnly(_options(), path, &db);
        //} else {
        //    s = shannon::DB::Open(_options(), path, &db);
        //}
        std::string driverpath = "/dev/kvdev0";
        s = shannon::DB::Open(_options(), path, driverpath, &db);
        invariantRocksOK(s);
        _db.reset(db);
        ...
        // open iterator
        std::unique_ptr<shannon::Iterator> iter(_db->NewIterator(shannon::ReadOptions()));
    ...
    Status RocksEngine::createRecordStore(OperationContext* opCtx, StringData ns, StringData ident,
        ...
        if (s.isOK() && NamespaceString::oplog(ns)) {
            ...
            s = rocksToMongoStatus(
                _db->Put(shannon::WriteOptions(), encodedPrefix, shannon::Slice("0")));
        }
        return s;
    }
    ...
    Status RocksEngine::dropIdent(OperationContext* opCtx, StringData ident) {
        ...
        shannon::WriteOptions syncOptions;
    ...
    Status RocksEngine::backup(const std::string& path) {
        /*
        shannon::Checkpoint* checkpoint;
        auto s = shannon::Checkpoint::Create(_db.get(), &checkpoint);
        if (s.ok()) {
    ...
    Status RocksEngine::_createIdent(StringData ident, BSONObjBuilder* configBuilder) {
        ...
        auto s = _db->Put(shannon::WriteOptions(), kMetadataPrefix + ident.toString(),
                          shannon::Slice(config.objdata(), config.objsize()));

        if (s.ok()) {
            // As an optimization, add a key <prefix> to the DB
            std::string encodedPrefix(encodePrefix(prefix));
            s = _db->Put(shannon::WriteOptions(), encodedPrefix, shannon::Slice("0"));
        }
    ...
    shannon::Options RocksEngine::_options() const {
        // default options
        shannon::Options options;
        //options.rate_limiter = _rateLimiter;
        // shannon::BlockBasedTableOptions table_options;
        // table_options.block_cache = nullptr;//_block_cache;
        // table_options.filter_policy.reset(shannon::NewBloomFilterPolicy(10));
        // table_options.block_size = 16 * 1024; // 16KB
        //table_options.format_version = 2;
        // options.table_factory.reset(shannon::NewBlockBasedTableFactory(table_options));
	...
	/*
        ...
        options.compression_per_level[0] = shannon::kNoCompression;
        options.compression_per_level[1] = shannon::kNoCompression;
        if (rocksGlobalOptions.compression == "snappy") {
            options.compression_per_level[2] = shannon::kSnappyCompression;
        } else if (rocksGlobalOptions.compression == "zlib") {
            options.compression_per_level[2] = shannon::kZlibCompression;
        } else if (rocksGlobalOptions.compression == "none") {
            options.compression_per_level[2] = shannon::kNoCompression;
        } else if (rocksGlobalOptions.compression == "lz4") {
            options.compression_per_level[2] = shannon::kLZ4Compression;
        } else if (rocksGlobalOptions.compression == "lz4hc") {
            options.compression_per_level[2] = shannon::kLZ4HCCompression;
        } else {
	...
	*/
        ...
        if (!rocksGlobalOptions.configString.empty()) {
            shannon::Options base_options(options);
            // TODO: 需要哪些Option先写死
            /*
            auto s = shannon::GetOptionsFromString(base_options, rocksGlobalOptions.configString,
                                                   &options);
            if (!s.ok()) {
