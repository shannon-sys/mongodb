...
namespace mongo {
    ...
    void RocksOptionsParameter::append(OperationContext* opCtx, BSONObjBuilder& b,
        ...
        shannon::Options fullOptions = _engine->getDB()->GetOptions();
        /*shannon::Status s = GetStringFromColumnFamilyOptions(&columnOptions, fullOptions);
...
//#if defined(ROCKSDB_MAJOR) && (ROCKSDB_MAJOR > 4 || (ROCKSDB_MAJOR == 4 && ROCKSDB_MINOR >= 13))        
        ...
        /*shannon::Status s = shannon::StringToMap(str, &optionsMap);
        if (!s.ok()) {
