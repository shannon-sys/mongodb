...
namespace mongo {
    ...
    public:
        ...
        shannon::CompactionFilterFactory* createCompactionFilterFactory() const;
        std::unordered_set<uint32_t> getDroppedPrefixes() const;
        uint32_t loadDroppedPrefixes(shannon::Iterator* iter);
        Status dropPrefixesAtomic(const std::vector<std::string>& prefixesToDrop,
                                  const shannon::WriteOptions& syncOptions,
