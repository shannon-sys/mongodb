...
namespace mongo {
    ...
    // Same as shannon::Iterator, but adds couple more useful functions
    class RocksIterator : public shannon::Iterator {
    public:
        ...
        virtual void SeekPrefix(const shannon::Slice& target) = 0;
    };
    ...
    public:
        ...
        shannon::WriteBatchWithIndex* writeBatch();

        const shannon::Snapshot* getPreparedSnapshot();
        void dbReleaseSnapshot(const shannon::Snapshot* snapshot);
        ...
        shannon::Status Get(const shannon::Slice& key, std::string* value);

        RocksIterator* NewIterator(std::string prefix, bool isOplog = false);

        static RocksIterator* NewIteratorNoSnapshot(shannon::DB* db, std::string prefix);

        void incrementCounter(const shannon::Slice& counterKey,
                              std::atomic<long long>* counter, long long delta);

        long long getDeltaCounter(const shannon::Slice& counterKey);
        ...
        void setCommittedSnapshot(const shannon::Snapshot* committedSnapshot);

        shannon::DB* getDB() const { return _db; }
