...
namespace mongo {
    ...
    namespace {
        ...
        public:
            RocksCursorBase(OperationContext* opCtx, shannon::DB* db, std::string prefix,
        ...
        protected:
            ...
            void advanceCursor() {
                ...
                if (_iterator.get() == nullptr) {
                    _iterator.reset(RocksRecoveryUnit::getRocksRecoveryUnit(_opCtx)
                            ->NewIterator(_prefix));
                    _iterator->SeekPrefix(shannon::Slice(_key.getBuffer(), _key.getSize()));
            ...
            bool seekCursor(const KeyString& query) {
                auto * iter = iterator();
                const shannon::Slice keySlice(query.getBuffer(), query.getSize());
                iter->Seek(keySlice);
            ...
            shannon::Slice _valueSlice() {
                if (_iterator.get() == nullptr) {
                    return shannon::Slice(_value);
                }
                return shannon::Slice(_iterator->value());
            }
            ...
            shannon::SequenceNumber _currentSequenceNumber;
        ...
        public:
            RocksStandardCursor(OperationContext* opCtx, shannon::DB* db, std::string prefix,
                                bool forward, Ordering order, KeyString::Version keyStringVersion)
        ...
        public:
            RocksUniqueCursor(OperationContext* opCtx, shannon::DB* db, std::string prefix,
                              bool forward, Ordering order, KeyString::Version keyStringVersion,
    ...
    private:
        void doInsert() {
            ...
            shannon::Slice valueSlice(value.getBuffer(), value.getSize());
    ...
    RocksIndexBase::RocksIndexBase(shannon::DB* db, std::string prefix, std::string ident,
          ...
          _order(order)
    {
        ...
        // 没提供接口，应该只影响统计展示，先用0
        //shannon::Range wholeRange(_prefix, nextPrefix);
    ...
    bool RocksIndexBase::isEmpty(OperationContext* opCtx) {
        ...
        std::unique_ptr<shannon::Iterator> it(ru->NewIterator(_prefix));
    ...
    RocksUniqueIndex::RocksUniqueIndex(shannon::DB* db, std::string prefix, std::string ident,
                                       Ordering order, const BSONObj& config,
    ...
    Status RocksUniqueIndex::insert(OperationContext* opCtx, const BSONObj& key, const RecordId& loc,
        ...
        shannon::Slice valueVectorSlice(valueVector.getBuffer(), valueVector.getSize());
    ...
    void RocksUniqueIndex::unindex(OperationContext* opCtx, const BSONObj& key, const RecordId& loc,
        ...
        shannon::Slice newValueSlice(newValue.getBuffer(), newValue.getSize());
    ...
    /// RocksStandardIndex
    RocksStandardIndex::RocksStandardIndex(shannon::DB* db, std::string prefix, std::string ident,
                                           Ordering order, const BSONObj& config)
        : RocksIndexBase(db, prefix, ident, order, config),
    ...
    Status RocksStandardIndex::insert(OperationContext* opCtx, const BSONObj& key,
        ...
        if (!encodedKey.getTypeBits().isAllZeros()) {
            value =
                shannon::Slice(reinterpret_cast<const char*>(encodedKey.getTypeBits().getBuffer()),
