...
namespace mongo {
    ...
    inline std::string rocksGetNextPrefix(const shannon::Slice& prefix) {
    ...
    bool extractPrefix(const shannon::Slice& slice, uint32_t* prefix);
    int get_internal_delete_skipped_count();

    Status rocksToMongoStatus_slow(const shannon::Status& status, const char* prefix);

    /**
     * converts rocksdb status to mongodb status
     */
    inline Status rocksToMongoStatus(const shannon::Status& status, const char* prefix = NULL) {
