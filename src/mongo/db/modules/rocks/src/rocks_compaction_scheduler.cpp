...
namespace mongo {
    namespace {
        ...
        class PrefixDeletingCompactionFilter : public shannon::CompactionFilter {
        public:
            ...
            virtual bool Filter(int level, const shannon::Slice& key,
                                const shannon::Slice& existing_value, std::string* new_value,
...
//#endif
        ...
        class PrefixDeletingCompactionFilterFactory : public shannon::CompactionFilterFactory {
        public:
            ...
            virtual std::unique_ptr<shannon::CompactionFilter> CreateCompactionFilter(
                ...
                if (droppedPrefixes.size() == 0) {
                    ...
                    return std::unique_ptr<shannon::CompactionFilter>(nullptr);
                } else {
    ...
    public:
        CompactionBackgroundJob(shannon::DB* db, RocksCompactionScheduler* compactionScheduler);
    ...
    CompactionBackgroundJob::CompactionBackgroundJob(shannon::DB* db,
...
//        shannon::CancelAllBackgroundWork(_db);
//#endif
    ...
    void CompactionBackgroundJob::compact(const CompactOp& op) {
        shannon::Slice start_slice(op._start_str);
        shannon::Slice end_slice(op._end_str);

        shannon::Slice* start = !op._start_str.empty() ? &start_slice : nullptr;
        shannon::Slice* end = !op._end_str.empty() ? &end_slice : nullptr;
        ...
        // 删除DeleteFilesInRange
        //if (op._rangeDropped) {
        //    auto s = shannon::DeleteFilesInRange(_db, _db->DefaultColumnFamily(), start, end);
        //    if (!s.ok()) {
        ...
        //compact_options.bottommost_level_compaction = shannon::BottommostLevelCompaction::kForce;
    ...
    void RocksCompactionScheduler::start(shannon::DB* db) {
        _db = db;
        _timer.reset();
    ...
    void RocksCompactionScheduler::reportSkippedDeletionsAboveThreshold(const std::string& prefix) {
        ...
        if (schedule) {
            log() << "Scheduling compaction to clean up tombstones for prefix "
                  << shannon::Slice(prefix).ToString(true);
    ...
    shannon::CompactionFilterFactory* RocksCompactionScheduler::createCompactionFilterFactory()
        const {
    ...
    uint32_t RocksCompactionScheduler::loadDroppedPrefixes(shannon::Iterator* iter) {
        invariant(iter);
    ...
    Status RocksCompactionScheduler::dropPrefixesAtomic(
        const std::vector<std::string>& prefixesToDrop, const shannon::WriteOptions& syncOptions,
        ...
        for (const auto& prefix : prefixesToDrop) {
            wb.Put(kDroppedPrefix + prefix, "0");
        }
